...
# Split the hurr_tracks into two lists: metadata and the data of each observation ----
...
east_states <- c("florida", "georgia", "south carolina", "north carolina", "virginia", "maryland", "delaware", "new jersey", "new york", "connecticut", "massachusetts", "rhode island", "vermont", "new hampshire", "maine", "pennsylvania", "west virginia", "tennessee", "kentucky", "alabama", "arkansas", "texas", "mississippi", "louisiana")
...
ggplot(east_us, aes(x = long, y = lat, group = group)) +
	...
	geom_path(data = hurr_obs_named,
			  aes(x = -longitude, y = latitude,
			  	group = storm_id),
			  color = "red", alpha = 0.2, size = 0.2) +
	# geom_path(data = filter(hurr_obs_named, cat_5),
	# 		  aes(x = -longitude, y = latitude,
	# 		  	group = storm_id),
	# 		  color = "red") +
	facet_wrap(~ decade)
