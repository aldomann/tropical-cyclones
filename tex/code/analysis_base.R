...
# SRC: https://stackoverflow.com/a/33302968/988432
scale_x_longitude <- function(xmin = -180, xmax = 180, step = 1, xtra.lim = 1.5, ...) {
	...
	return(scale_x_continuous("Longitude", breaks = xbreaks, labels = xlabels, expand = c(0, 0), limits = c(xmin-xtra.lim, xmax+xtra.lim), ...))
}

scale_y_latitude <- function(ymin = -90, ymax = 90, step = 0.5, xtra.lim = 1.5, ...) {
	ybreaks <- seq(ymin,ymax,step)
	ylabels <- unlist(lapply(ybreaks, function(x) ifelse(x < 0, parse(text=paste0(-x,"^o", "*S")), ifelse(x > 0, parse(text=paste0(x,"^o", "*N")), x))))
	return(scale_y_continuous("Latitude", breaks = ybreaks, labels = ylabels, expand = c(0, 0), limits = c(ymin-xtra.lim, ymax+xtra.lim), ...))
}
...
map_region_hurrs <- function(hurr.obs, years, coords, rect.coords, steps = c(5,5), xtra.lims = c(1.5,1.5)){
	...
	ggplot(data = world_map, aes(x = long, y = lat, group = group)) +
		...
		scale_x_longitude(xmin = as.numeric(coords[1]),
											xmax = as.numeric(coords[2]),
											step = steps[1], xtra.lim = xtra.lims[1]) +
		scale_y_latitude(ymin = as.numeric(coords[3]),
										 ymax = as.numeric(coords[4]),
										 step = steps[2], xtra.lim = xtra.lims[2]) +
		coord_proj("+proj=merc") +
		geom_path(data = hurr.obs, aes(x = long, y = lat, group = storm.id), colour = "red", alpha = 0.2, size = 0.2) +
		annotate("rect", xmin = as.integer(rect.coords[1]),
						 xmax = as.integer(rect.coords[2]),
						 ymin = as.integer(rect.coords[3]),
						 ymax = as.integer(rect.coords[4]),
...
plot_dpdi_by_sst_class <- function(hurr.obs.pdi, ssts.df){
	...
	ggplot() +
		...
		labs(title = paste0("PDI probability density for ",
												...
												" (", attr(ssts.df, "title"), ")"),
				 x = bquote(PDI~ (m^3 ~s^-2)),
				 y = bquote(D(PDI)~(s^2~m^-3)),
				 colour = "SST class")
}
...
plot_pdi_scatter_by_class <- function(hurr.pdi, ssts, class){
	...
	if(class == "high"){
		gg +
			scale_colour_manual(labels = c(bquote(.(paste0("developing; ")) ~ r^2 ~ .(paste0("= ", format(summary(lm.ds.y)$r.squared, digits = 2)))), bquote(.(paste0("non-developing; ")) ~ r^2 ~ .(paste0("= ", format(summary(lm.nds.y)$r.squared, digits = 2))))),
...
plot_pdi_scatter_by_status <- function(hurr.pdi, ssts, status){
	...
	gg <- ggplot() +
		...
		guides(colour = guide_legend(order = 1, override.aes = list(linetype = c(0,0), shape = c(1,5))),
					 linetype = guide_legend(order = 2, override.aes = list(colour = c("black", "black")))) +
		scale_colour_manual(labels = c(bquote(.(paste0("high; ")) ~ r^2 ~ .(paste0("= ", format(summary(lm.high.y)$r.squared, digits = 2)))), bquote(.(paste0("low;  ")) ~ r^2 ~ .(paste0("= ", format(summary(lm.low.y)$r.squared, digits = 2))))),
