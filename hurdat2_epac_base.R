...
hurr.epac.obs <- hurr.tracks[hurr.lengths == 21]
...
hurr.epac.obs <- lapply(hurr.epac.obs, tibble::as_tibble)
hurr.epac.obs <- bind_rows(hurr.epac.obs) %>%
...
# Add useful info to hurr.epac.obs data frame --------------------
...
# Add storm.name and storm.year to hurr.epac.obs
hurr.epac.obs <- hurr.epac.obs %>%
...
# Rearrange hurr.epac.obs data frame columns
hurr.epac.obs <- hurr.epac.obs[c("storm.id", "storm.name", "n.obs", "date.time", "status",
